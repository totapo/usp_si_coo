...
public class Main {
	...
	public static void main(String [] args){
		...
		while(running){
			...
			timer.getCurrentTime() = System.timer.getCurrentTime()Millis();
			...
			if(player_state == ACTIVE){
				...
				for(int i = 0; i < e_projectile_states.length; i++){
					...
					if(dist < (player_radius + e_projectile_radius) * 0.8){
						...
						player_explosion_start = timer.getCurrentTime();
						player_explosion_end = timer.getCurrentTime() + 2000;
					}
				}
				...
				for(int i = 0; i < enemy1_states.length; i++){
					...
					if(enemy1_states[i] == ACTIVE){
						...
						if(dist < enemy1_radius){
							...
							enemy1_explosion_start[i] = timer.getCurrentTime();
							enemy1_explosion_end[i] = timer.getCurrentTime() + 500;
						}
					}
				}
				...
				for(int i = 0; i < enemy2_states.length; i++){
					...
					if(enemy2_states[i] == ACTIVE){
						...
						if(dist < enemy2_radius){
							...
							enemy2_explosion_start[i] = timer.getCurrentTime();
							enemy2_explosion_end[i] = timer.getCurrentTime() + 500;
						}
					}
				}
			}
			...
			for(int i = 0; i < enemy1_states.length; i++){
				...
				if(enemy1_states[i] == EXPLODING){
					...
					if(timer.getCurrentTime() > enemy1_explosion_end[i]){
				...
				if(enemy1_states[i] == ACTIVE){
					...
					if(enemy1_Y[i] > GameLib.HEIGHT + 10) {
						...
						if(timer.getCurrentTime() > enemy1_nextShoot[i] && enemy1_Y[i] < player_Y){
							...
							if(free < e_projectile_states.length){
								...
								enemy1_nextShoot[i] = (long) (timer.getCurrentTime() + 200 + Math.random() * 500);
							}
						}
					}
				}
			...
			for(int i = 0; i < enemy2_states.length; i++){
				...
				if(enemy2_states[i] == EXPLODING){
					...
					if(timer.getCurrentTime() > enemy2_explosion_end[i]){
			...
			if(timer.getCurrentTime() > nextEnemy1){
				...
				if(free < enemy1_states.length){
					...
					enemy1_nextShoot[free] = timer.getCurrentTime() + 500;
					nextEnemy1 = timer.getCurrentTime() + 500;
				}
			}
			
			/* verificando se novos inimigos (tipo 2) devem ser "lançados" */
			
			if(timer.getCurrentTime() > nextEnemy2){
				...
				if(free < enemy2_states.length){
					...
					if(enemy2_count < 10){
						...
						nextEnemy2 = timer.getCurrentTime() + 120;
					}
					else {
						
						enemy2_count = 0;
						enemy2_spawnX = Math.random() > 0.5 ? GameLib.WIDTH * 0.2 : GameLib.WIDTH * 0.8;
						nextEnemy2 = (long) (timer.getCurrentTime() + 3000 + Math.random() * 3000);
					}
				}
			}
			...
			if(player_state == EXPLODING){
				...
				if(timer.getCurrentTime() > player_explosion_end){
			...
			if(player_state == ACTIVE){
				...
				if(GameLib.iskeyPressed(GameLib.KEY_CONTROL)) {
					...
					if(timer.getCurrentTime() > player_nextShot){
						...
						if(free < projectile_states.length){
							...
							player_nextShot = timer.getCurrentTime() + 100;
						}
					}	
				}
			}
			...
			if(player_state == EXPLODING){
				...
				double alpha = (timer.getCurrentTime() - player_explosion_start) / (player_explosion_end - player_explosion_start);
			...
			for(int i = 0; i < enemy1_states.length; i++){
				...
				if(enemy1_states[i] == EXPLODING){
					...
					double alpha = (timer.getCurrentTime() - enemy1_explosion_start[i]) / (enemy1_explosion_end[i] - enemy1_explosion_start[i]);
			...
			for(int i = 0; i < enemy2_states.length; i++){
				...
				if(enemy2_states[i] == EXPLODING){
					...
					double alpha = (timer.getCurrentTime() - enemy2_explosion_start[i]) / (enemy2_explosion_end[i] - enemy2_explosion_start[i]);
